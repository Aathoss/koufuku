...
func LevelingMessages(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	t1 := time.Now()
	insert, err := framework.DBKoufuku.Prepare("INSERT INTO logs(uuid, categorie, timestamp, content) VALUES(?, ?, ?, ?)")
	if err != nil {
	...
	framework.DBKoufuku.QueryRow("SELECT niveau,xp FROM level WHERE uuid=?", lvl.uuid).Scan(&lvl.niveau, &lvl.xp)

	lvl.xp = lvl.xp + (rand.Intn(10) + 10)
	calculNiveau := (5 * lvl.niveau) + (75 * lvl.niveau) + 200
	calculNiveauUP := (5*lvl.niveau + 1) + (75*lvl.niveau + 1) + 200
	...
	insert, err = framework.DBKoufuku.Prepare(`
